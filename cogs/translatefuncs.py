...
def find_translation(
    ...
    try:
        if (
            targetlang != "key"
        ):  # if either are key, they should not be searched for as files, instead use jsdef
    ...
    if targetlang == "key":  # figures out, which mode to use
        ...
        if sourcelang == "key":
            ...
            try:
                exact = jstarget[string]
            except:
                pass
        else:
            result = [
                jstarget[i]
                for i in [i for i in jssource if string in jssource[i].lower()]
            ]  # sts(string, jstarget, jssource)
            for i in jssource:
                if jssource[i].lower() == string:
                    exact = jstarget[i]
                    break
    ...
    while (
        True
    ):  # iterates through all results to make sure lat page number is consistant
        k += 1
    ...
    if (
        len(result) > 10
    ):  # if there are more than 10 results, sets result to the 10 strings following (page-1)
        result = result[pagesize * (page - 1) : pagesize * page]
        added = True

    if added:
        buttons = di.ActionRow(prevbutton, nextbutton)
    else:
        buttons = None
...
def lang(search: str, edition):
    ...
    if edition == "java":
        for i in range(
            len(langcodesapp)
        ):  # We can't use complete here because we would have no clue which langcode to use. The thing we need is index of langcode, not completed langname or whatever.
...
def register_comp(id: int, search, target, source, edition, channel):
    with open(
        os.path.join(os.path.dirname(os.path.dirname(__file__)), "loadedmessages.json"),
        "r",
    ) as f:
        t = time.time()
        cont = json.load(f)
        ...
        json.dump(
            ...
            open(
                os.path.join(
                    os.path.dirname(os.path.dirname(__file__)), "loadedmessages.json"
                ),
                "w",
            ),
        )
